...
public class Player : MonoBehaviour
{
	...
	void Update() {
        ...
        // -- Handle Animations --
		// Attack animation
		if (Input.GetKeyDown("space") && m_TimeSinceAttack > 0.25f && !m_Rolling){
			m_CurrentAttack++;
		...
		// Roll animation
		else if (Input.GetKeyDown("s") && !m_Rolling){
			m_Rolling = true;
	...
	void Attack()
    {
        ...
        if (enemy.GetComponent<Enemy>().m_Rolling == false){
    ...
    public void Die(){
		...
		StartCoroutine(ChangeScene("TreePlayer2Win"));
